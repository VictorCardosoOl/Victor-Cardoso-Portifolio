...
// Adicione esta interface acima do componente Contact ou num arquivo types.ts
...
const Contact: React.FC = () => {
  ...
  // Definição tipada do sub-componente
  const InputGroup: React.FC<InputGroupProps> = ({ id, label, type = "text", placeholder, value, required = false }) => {
    ...
    return (
      <div className="relative group pb-4">
        <label
          htmlFor={id}
          className={`block text-[10px] font-bold uppercase tracking-widest mb-1 transition-colors duration-300 ${hasError ? 'text-red-500' : focusedField === id ? 'text-slate-900' : 'text-slate-400'
            }`}
        >
        ...
        <div className="relative">
          <input
            ...
            className={`w-full bg-transparent border-b py-3 px-2 text-base text-slate-900 placeholder-slate-300 focus:outline-none transition-all duration-300 rounded-t-md ${hasError
          ...
          />
          <div
            className={`absolute bottom-0 left-0 h-[2px] bg-slate-900 transition-all duration-500 ease-out z-10 ${focusedField === id && !hasError ? 'w-full' : 'w-0'
              }`}
          />
          <div
            className={`absolute bottom-0 left-0 h-[2px] bg-red-500 transition-all duration-500 ease-out z-10 ${hasError ? 'w-full' : 'w-0'
              }`}
          />
        </div>
        {hasError && (
              ...
              <div className="glass-panel p-8 md:p-10 rounded-[2rem] relative overflow-hidden">
                ...
                <form onSubmit={handleSubmit} noValidate className="space-y-8 relative z-10">
                  ...
                  <div className="relative group pb-4">
                    <label
                      htmlFor="message"
                      className={`block text-[10px] font-bold uppercase tracking-widest mb-1 transition-colors duration-300 ${errors.message ? 'text-red-500' : focusedField === 'message' ? 'text-slate-900' : 'text-slate-400'
                        }`}
                    >
                      Mensagem *
                    </label>
                    <div className="relative">
                      <textarea
                        ...
                        onBlur={() => setFocusedField(null)}
                        className={`w-full bg-transparent border-b py-3 px-2 text-base text-slate-900 placeholder-slate-300 focus:outline-none transition-all duration-300 resize-none rounded-t-md ${errors.message ? 'border-red-400 bg-red-50/50' : 'border-slate-200 hover:bg-slate-50'
                          }`}
