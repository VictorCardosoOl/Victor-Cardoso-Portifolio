              ...
              <div className="glass-panel p-6 md:p-10 rounded-[2rem] relative overflow-hidden bg-white/50 border border-white/20 min-h-[500px] flex flex-col justify-center">
                ...
                <form onSubmit={handleSubmit} noValidate className={`space-y-6 md:space-y-8 relative z-10 transition-opacity duration-300 ${status === 'success' ? 'opacity-0' : 'opacity-100'}`}>
                  ...
                  <div className="relative group pb-6">
                    <label
                      htmlFor="message"
                      className={`block text-[9px] md:text-[10px] font-bold uppercase tracking-widest mb-1 transition-colors duration-300 ${errors.message ? 'text-red-600' : focusedField === 'message' ? 'text-petrol-base' : 'text-petrol-base/40'
                        }`}
                    >
                      Mensagem *
                    </label>
                    <div className="relative">
                      <textarea
                        ...
                        className={`w-full bg-transparent border-b-2 py-3 px-2 text-base placeholder-petrol-base/20 focus:outline-none transition-all duration-300 resize-none rounded-t-md ${errors.message
...
const InputGroup = ({
  ...
  return (
    <div className="relative group pb-6">
      <label
        htmlFor={id}
        className={`block text-[9px] md:text-[10px] font-bold uppercase tracking-widest mb-1 transition-colors duration-300 ${hasError ? 'text-red-600' : focusedField === id ? 'text-petrol-base' : 'text-petrol-base/40'
          }`}
      >
        {label} {required && '*'}
      </label>
      <div className="relative">
        <input
          ...
          onBlur={() => setFocusedField(null)}
          className={`w-full bg-transparent border-b-2 py-3 px-2 text-base placeholder-petrol-base/20 focus:outline-none transition-all duration-300 rounded-t-md ${hasError
              ? 'border-red-600 bg-red-100 text-red-900 placeholder-red-300'
              : 'border-petrol-base/10 text-petrol-base hover:bg-white focus:border-petrol-base'
            }`}
        ...
        {!hasError && (
          <div
            className={`absolute bottom-[-2px] left-0 h-[2px] bg-petrol-base transition-all duration-500 ease-out z-10 ${focusedField === id ? 'w-full' : 'w-0'
              }`}
          />
        )}
      </div>
