    ...
    <div ref={containerRef} className="group py-12 md:py-20 lg:py-32 border-b border-slate-200 last:border-0 relative">
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 md:gap-12 lg:gap-20 items-start">
        ...
        <div className="lg:col-span-5 lg:sticky lg:top-32 flex flex-col h-full justify-between order-2 lg:order-1">
          <Reveal width="100%">
            {/* Subtle Scale Animation inherited from new Reveal */}
            ...
            <h3 className="text-4xl md:text-5xl lg:text-8xl font-serif font-medium text-slate-900 mb-6 md:mb-8 leading-[1] tracking-tighter">
              {project.title}
            </h3>
            ...
            <div className="flex flex-col sm:flex-row items-start sm:items-center gap-4 sm:gap-6">
              <button
                onClick={() => { toggleCaseStudy(); unlockArchitect(); }}
...
const Projects: React.FC = () => {
  ...
  // Trigger VISIONARY when component mounts (user reaches projects section)
  useEffect(() => {
    const observer = new IntersectionObserver((entries) => {
      if (entries[0].isIntersecting) {
        unlockAchievement('VISIONARY');
      }
    }, { threshold: 0.2 });
      ...
      <div className="container mx-auto px-6 md:px-12 xl:px-20">
        {PROJECTS.map((project, index) => (
          <ProjectRow
            ...
            unlockArchitect={() => unlockAchievement('ARCHITECT')}
          />
        ))}
      </div>
